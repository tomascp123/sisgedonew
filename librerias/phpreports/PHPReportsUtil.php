...
// esta linea crea un archivo temporal en la carpeta de windows
		$sPath = tempnam("/tmp","check");
		unlink($sPath);
	...
	******************************************************************************/
	class PHPReportsError{
		function PHPReportsError($sMsg_=null,$sURL_=null){
			...
			print "<span style='font-size:18px;color:#FF0000;font-weight:bold;'>MENSAJE DEL SISTEMA</span><br/><br/>";
			print $sMsg_."<br/><br/>";
			?>
				<a href="javascript: window.opener=''; window.close();">Cerrar Ventana</a>			
			<?
			if(!is_null($sURL_))
