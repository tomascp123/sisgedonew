...
<form name="<? echo $_nameform ?>" method="post" action="<? echo $_url?>?_tipoedicion=<? echo $_tipoedicion?>&_op=<? echo $_op?>&_type=<? if ($_tipoedicion==1 or $_tipoedicion==2 or $_tipoedicion==5) echo 'G&_nametype=php_grabarmanual.php'; else echo 'M'; ?>&_tabactivo=<? echo $_tabactivo ?>&pagina=<? echo $_pagina ?>&orden=<? echo $_orden ?>&_where=<? echo $_where ?>&_npop=<? echo $_npop ?>&_flag=<? echo $_flag ?>" onSubmit="return <? echo iif($_tipoedicion,'<=',2,'ObligaCamposyDisabled(this)','true') ?> ">
	<tr>
		<td colspan="7" >
